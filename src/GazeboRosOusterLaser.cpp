...
void GazeboRosOusterLaser::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  if (!_sdf->HasElement("organize_cloud")) {
    ROS_INFO("Ouster laser plugin missing <organize_cloud>, defaults to false");
  ...
  if (!_sdf->HasElement("min_intensity")) {
    ROS_INFO("Ouster laser plugin missing <min_intensity>, defaults to no clipping");
  } else {
...
void GazeboRosOusterLaser::OnScan(ConstLaserScanStampedPtr& _msg)
{
  ...
  for (i = 0; i < verticalRangeCount; i++) {
    for (j = 0; j < rangeCount; j++) {
      // Range
      double r = _msg->scan().ranges(j + i * rangeCount);
      // Intensity
      double intensity = _msg->scan().intensities(j + i * rangeCount);
      ...
      if (rangeCount > 1) {
        yAngle = j * yDiff / (rangeCount -1) + minAngle.Radian();
      } else {
        yAngle = minAngle.Radian();
      }

      if (verticalRayCount > 1) {
        pAngle = i * pDiff / (verticalRangeCount -1) + verticalMinAngle.Radian();
      } else {
      ...
      if ((MIN_RANGE < r) && (r < MAX_RANGE)) {
        ...
        *((uint32_t*)(ptr + 16)) = 0.0; // t
        *((uint16_t*)(ptr + 20)) = 0.0; // reflectivity
        *((uint8_t*)(ptr + 22)) = i; // ring
        ptr += POINT_STEP;
  ...
  // std::cout<<"it's me. "<<parent_ray_sensor_->LastUpdateTime().Double()<<std::endl;
  // Publish output
  pub_.publish(msg);
}
